...
class ForgeApp(ForgeBase):
    ...
    def get_issues(self, container_id):
        issues = self.api.hq.get_issues(container_id)
        return issues

    @_validate_bim360_hub
    def get_ish(self, issues_id):
        self.issues = self.api.hq.get_iss_test(issues_id)
        self._issues_indices_by_name = {
            issues["name"]: i for i, issues in enumerate(self.issues)
        }
