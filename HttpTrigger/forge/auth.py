...
class ForgeAuth(ForgeBase):
    ...
    def _get_token(self):
        ...
        headers = {"Content-Type": "application/x-www-form-urlencoded"}
        logger.info("params in self: %s", self.__dict__.keys())
        params = {
            "client_id": self.client_id,
            "client_secret": self.client_secret,
    ...
    def _set_auth_header(self):
        logger.info("Set header options: %s",self.__dict__.keys())
        ...
        try:
            ...
            logger.info(self.token_type)
            logger.info(self.access_token)
        except Exception as e:
            raise AttributeError("Failed to get Bearer Token: " + str(e))
